  ...
  useEffect(() => { 
    const getCards = async () => {
      try {
        ...
        let cardList;
        if(user?.role === "Client"){
          cardList = res.data.filter((card: Card) => card?.user?.id === user?.id);
        } else {
          cardList = res.data;
        }
        setCards(cardList);
      ...
      <main className="p-2 mb-10">
...

        {
          ...
          ? <div className="grid grid-cols-1 gap-3">
            {
              cards?.map((card) => (
                <Link key={card?.id} href={`/cards/${card?.id}`} className={`flex justify-between items-center shadow-lg rounded-lg p-5 ${card?.maxDays <= card?.mises?.length && 'bg-green-500'}`}>
                    <div className="user">
                    ...
                    <div className="amount">
                      <span className="text-2xl font-semibold text-center">{card?.montant}{card?.devise === "CDF" ? "Fc" : "$"}</span>
                    </div>
                </Link>
              ))
            }
